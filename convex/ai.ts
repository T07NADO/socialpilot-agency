...
export const generatePost = action({
  args: {
    ...
    personContext: v.optional(v.string()),
  },
  handler: async (_ctx, args) => {
    const Groq = (await import("groq-sdk")).default;
    const groq = new Groq({ apiKey: process.env.GROQ_API_KEY });

    const brandContext = args.personContext
      ? args.personContext
      : args.brandVoice
      ? `Brand Voice:
...
${args.brandVoice.examplePosts.length ? `\nExample posts:\n${args.brandVoice.examplePosts.map((p, i) => `${i + 1}. ${p}`).join("\n")}` : ""}`
      : `Writing for ${args.clientName}, a ${args.industry} professional.`;
    ...
    const prompt = `You are a social media ghostwriter writing as a real person, not a brand.
...
Write in first person. Sound like the person, not like a marketing copywriter. Avoid cliches.
